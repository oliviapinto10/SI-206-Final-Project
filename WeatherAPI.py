...
API_KEY = "oRplhBpTnsEwrYHBzhCFBCTObaJrFYyU"
   ...
   while last_date and data_count < 25:
       ...
       for item in data:
           ...
           if date.date() >= last_date.date() and temperature is not None:
               cursor.execute("INSERT OR REPLACE INTO temperature_data VALUES (?, ?)", (date.strftime("%Y-%m-%d"), temperature))
               data_count += 1
               last_date = date
               print(f"Inserted data point {data_count}: {date.strftime('%Y-%m-%d')}, {temperature}°C")
